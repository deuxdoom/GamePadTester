...
def compute_polling_stats(intervals_ns: List[int]) -> dict:
    """주어진 시간 간격 리스트(나노초)로부터 폴링 통계를 계산합니다."""
...
class GamepadWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # --- Triggers & Shoulders ---
        trigger_h, trigger_w = 65, 35
        shoulder_h, shoulder_w = 32, w * 0.17
        ...
        self._draw_trigger(painter, colors, lt_rect, self.trigger_L_val, "LT")
        self._draw_shoulder_button(painter, colors, lb_rect, self.button_states.get("LB", False), "LB")
        self._draw_trigger(painter, colors, rt_rect, self.trigger_R_val, "RT")
        self._draw_shoulder_button(painter, colors, rb_rect, self.button_states.get("RB", False), "RB")
        ...
        abxy_radius = 32
        draw_face_button(abxy_x_base, abxy_y_base - abxy_radius, "Y", "Y")
        draw_face_button(abxy_x_base - abxy_radius, abxy_y_base, "X", "X")
        draw_face_button(abxy_x_base + abxy_radius, abxy_y_base, "B", "B")
        draw_face_button(abxy_x_base, abxy_y_base + abxy_radius, "A", "A")

        # D-Pad
        dpad_x_base = c.x() - w * 0.27
        dpad_y_base = abxy_y_base
        arm_w, arm_l, gap = 30, 30, 15
        ...
        dpad_states = {
            "DPAD_UP": rect_up, "DPAD_DOWN": rect_down,
            "DPAD_LEFT": rect_left, "DPAD_RIGHT": rect_right
        }
        painter.setPen(QPen(C_BORDER, 1))
        ...
        draw_face_button(c.x() - 50, c.y() - h * 0.15, "BACK", "⁝")
        draw_face_button(c.x() + 50, c.y() - h * 0.15, "START", "≡")
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # Timer for smooth UI updates (~60 FPS)
        self._ui_timer = QTimer(self)
        self._ui_timer.setInterval(16)
        self._ui_timer.timeout.connect(self.update_gamepad_ui)
        self._ui_timer.start()

        # Timer for checking device connections (less frequent)
        self._connection_timer = QTimer(self)
        self._connection_timer.setInterval(500)
        self._connection_timer.timeout.connect(self.check_connection_status_realtime)
